...
module.exports = {
    ...
    tweet: function(user) {
        ...
        // A hash is needed to avoid the error "status is duplicate"
        var toHash = user.id + '' + (new Date().getTime()),
            hash = crypto.createHash('md5').update(toHash, 'utf-8').digest('hex');

        var params = {
                status: this.config.message +' '+ this.config.server.domain + '/hash/' + hash
            };
